                ...
                {!profile ? <Spinner /> : <>
                    ...
                    <div className={`${styles.profileCard} ${isDarkMode ? styles.profileCardDark : ""}`}>
                        <div className={styles.editButton} onClick={() => { setEdit(!edit) }}>
                            {!edit ? <i className="fa fa-pencil" aria-hidden="true"></i> : <i className="fa fa-times" aria-hidden="true"></i>}
                        </div>
                        <div className={styles.profilePic}>
                            {uploading ? <Spinner /> :
                                <>
                                    <div className={styles.profilePicImage}>
                                        {image ? <img src={image} alt="profile-image" /> : <i className={`fa fa-user-circle ${styles.userIcon}`} aria-hidden="true"></i>}
                                    </div>
                                    {edit && <>
                                        <input id="file-upload" type="file" accept="image/*" />
                                        <i className={`fa fa-camera ${styles.camera}`} aria-hidden="true" onClick={uploadHandler}></i>
                                    </>}
                                </>
                            }
                        </div>
